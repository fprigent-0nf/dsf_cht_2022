...
def parse_csv(path=CSV_PATH):
    ...
    df.columns = [
        f"{x[0]}{int(x[1]):0>2}".replace("MORBRA2___ARBRE", "MORTAL").replace(
            "PERRAM2___ARBRE", "MRAMIF"
        )
        for x in df.columns
    ]
    ...
    row = [
        ...
        3,
        "OUI",
        "releves_chataignier_IdF_DSF_2022_TB3.csv",
        5,
        5,
        5,
        5,
    ...
    gdf = gpd.GeoDataFrame(
        df, geometry=gpd.points_from_xy(df.x93, df.y93, crs="epsg:2154")
    )
    gdf.rename(
        columns={
            "code_placette": "NUM_PLAC",
