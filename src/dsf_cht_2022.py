...
tbgdf = TB.parse_csv()
# load csv
dcsv = pd.read_csv(CSV_PATH + "/dsf_cht_2022_p173_ronqueux_releve.csv", delimiter=";")
...
l1 = len(dsf)
dsf.drop_duplicates(
    subset=dsf.columns[dsf.columns != "filename"], keep="last", inplace=True
)
...
dsf = dsf.sjoin_nearest(
    rdf[["CCOD_FRT", "CCOD_UG", "CCOD_PRF", "geometry"]], how="left", max_distance=25
)
placettes_2022 = placettes_2022.sjoin_nearest(
    dsf[["NUM_PLAC", "geometry"]], how="left", distance_col="distance", max_distance=52
)
del dsf["index_right"]
...
arbres = arbres.pivot(
    index=["uuid", "NUM_PLAC", "NMASSIF", "num_arbre"],
    columns="variable",
    values="value",
).reset_index()
arbres = arbres.rename(
    columns={"MORTAL": "MB", "MRAMIF": "MR", "VISIH_": "visi"}, errors="raise"
)
...
lab = ["0", "1", "2", "3"]
ddl["etat_sanit"] = pd.to_numeric(
    pd.cut(ddl.prop_EF, bins=interv, labels=lab, include_lowest=True, right=False)
)
...
epigdf.to_csv(
    f"{OUT_PATH}/dsf_2022_epicollect.csv", index=False, sep=CSV_SEP, encoding="latin1"
)
...
nb_plac_par_notateur = (
    dsf[["uuid", "NOTATEUR"]].groupby("NOTATEUR").count().reset_index()
)
...
_dpi = 90
bin_20 = [0, 0.2, 0.4, 0.6, 0.8, 1]
bin_25 = [0, 0.25, 0.5, 0.75, 1]
...
g_20_EF = sns.displot(
    ddl, x="prop_EF", stat="count", bins=bin_20, hue="NMASSIF", multiple="stack"
)
...
#
g_20_DEF = sns.displot(
    ddl, x="prop_DEF", stat="count", bins=bin_20, hue="NMASSIF", multiple="stack"
)
...
g_25_EF = sns.displot(
    ddl, x="prop_EF", stat="count", bins=bin_25, hue="NMASSIF", multiple="stack"
)
...
#
g_25_DEF = sns.displot(
    ddl, x="prop_DEF", stat="count", bins=bin_25, hue="NMASSIF", multiple="stack"
)
...
g_etat_san_par_class.savefig(f"{IMG_PATH}/g_etat_san_par_class.png", dpi=_dpi)
...
g_rmax = sns.displot(dsf, x="R_MAX", stat="count", hue="NMASSIF", multiple="stack")
...
### visi houppier  #############################################################
g_visi = sns.displot(arbres, x="visi", stat="count", hue="NMASSIF", multiple="stack")
...
g_mb_mr = sns.relplot(
    ...
    y="MB",
    size="value",
    sizes=(min(mb_mr.value * 1.5), max(mb_mr.value * 1.5)),
    alpha=0.7,
    palette="muted",
    height=6,
    color="orange",
...
ax = dsf_wm.plot(
    figsize=(8, 14), alpha=0.9, color="red", markersize=1.8, edgecolor="red"
)
